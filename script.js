...
const ROUTES = {
  ...
  ARILI_PREMIUM: "./arilipremium.html"
};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const premiumBadgeLink = document.querySelector(".menu-user-badge");
  ...
  function toggleMenu(e) {
    ...
    if (!globalMenu || !menuToggle || isLoginRedirecting || isNavigating) return;
  ...
  function setLoggedInView(user) {
    ...
    if (menuUserState) {
      menuUserState.style.display = "grid";
    } else {
      if (menuUserEmail) menuUserEmail.textContent = user?.email || "";
      if (premiumBadgeLink) premiumBadgeLink.style.display = "inline-flex";
      if (menuLogoutButton) menuLogoutButton.style.display = "inline-flex";
    }
  ...
  async function handleLogin(e) {
    ...
    const submitButton = menuLoginForm
      ? menuLoginForm.querySelector('button[type="submit"], .menu-login-button, input[type="submit"]')
      : null;
